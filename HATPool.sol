...
contract HatPool{
    ...
    struct UserInfo{
        uint256 shares;             //当前质押OKT数量，若使用withdraw提取，则相应减少
        uint256 totalShares;        //已经质押OKT的总数量，包括邀请用户消耗的数量与当前质押数量
        uint256 rewardDebt;         //当前已经获得的奖励数量
        address payable invite;     //邀请人地址，若该地址为0地址，则没有邀请人
        uint256 active;             //是否已激活，用户完成质押后，激活此值，为1.
        uint256 lastRewardTime;     //最后一次获取奖励时间
        uint256 inviteCount;        //邀请人数
        uint256 free;               //免费提现hat额度，提取hat后，此额度会减少，若此额度为0，则每笔提现收取10%的hat的手续费
    }
    ...
    function withdrawHAT(uint256 _amount) public {
        ...
        if (_amount > user.free) {
            uint256 taxAmount = _amount.sub(user.free);
            uint256 taxFee = taxAmount.mul(10).div(100);
            uint256 realAmount = _amount.sub(taxFee);
            user.free = 0;
            hat.transfer(receiveAddress,taxFee);
            safeTransfer(msg.sender,realAmount);
        } else {
            user.free = user.free.sub(_amount);
